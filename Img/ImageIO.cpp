...
inline bool
strEq(const char* exp, const char* test)
{
#ifdef _WIN32
...
filename(filename)
{
	totalXRes = xRes = rgbaBuffer.m_sizeX;
	totalYRes = yRes = rgbaBuffer.m_sizeY;
...
Math::Color3f * 
readImage(const string &name, int *width, int *height)
{
...
bool 
writeImage(const ImageData &data)
{
    bool ok = false;
