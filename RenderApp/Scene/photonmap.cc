...
namespace {
	const double PI = 3.1415326539;
	const double ONE_265TH = (1.0 / 256.0);
}
...
PhotonMap::PhotonMap(const int max_phot) {
	...
	photons = std::unique_ptr<Photon[]>(new Photon[max_photons + 1]);
	...
	for (int i = 0; i < 256; i++) {
		const double angle = static_cast<double>(i) * ONE_265TH * PI;
		costheta[i] = static_cast<float>(cos(angle));
		sintheta[i] = static_cast<float>(sin(angle));
		cosphi[i]   = static_cast<float>(cos(2.0 * angle));
		sinphi[i]   = static_cast<float>(sin(2.0 * angle));
	}
}

PhotonMap::~PhotonMap() { } // Needs to be here for the destructor of photons
...
void PhotonMap::irradiance_estimate (
	...
	const float tmp = (1.0f / static_cast<float>(PI) ) / (np.dist2[0]);	// estimate of density
...
void PhotonMap::balance(void) {
	if (stored_photons > 1) {
		...
		auto pa1 = std::unique_ptr<Photon*[]>(new Photon*[stored_photons + 1]);
		auto pa2 = std::unique_ptr<Photon*[]>(new Photon*[stored_photons + 1]);

		for (int i = 0; i <= stored_photons; i++)
			pa2[i] = &photons[i];

		balance_segment(pa1.get(), pa2.get(), 1, 1, stored_photons);
		pa2.reset();
...
void PhotonMap::renderGL( ) {
	for (int i = 0; i < stored_photons; i++) {
		Photon cur = photons[i];
		glColor3f(cur.power[0] * stored_photons, cur.power[1] * stored_photons, cur.power[2] * stored_photons);
		glBegin(GL_POINTS);
		glVertex(Math::Vec3f(cur.pos));
		glEnd();
	}
}

int PhotonMap::getStoredPhotons() {
