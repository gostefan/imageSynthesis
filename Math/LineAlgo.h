...
namespace Math {
	...
	inline bool intersects(const Vec3f& ro, const Vec3f& rd, float time, const Box4f& box,
	...
	template <typename T>
	inline bool intersect(const Vec3<T>& ro, const Vec3<T>& rd,
			const Vec3<T>& A,
			const Vec3<T>& B,
			const Vec3<T>& C,
			T t0, T t1,
	...
	template <typename T>
	inline bool intersects(const Vec3<T>& ro, const Vec3<T>& rd,
			const Vec3<T>& A,
			const Vec3<T>& B,
			const Vec3<T>& C,
			T t0, T t1)
