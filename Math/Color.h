...
namespace Math {
	...
	class Color3 : public Vec3<T> {
		public:
			...
			const Color3<T>& operator+= (const Color3<T>& c) {
				*static_cast<Vec3<T>*>(this) += c;
				return *this;
			}
			const Color3<T>& operator+= (T a) {
				*static_cast<Vec3<T>*>(this) += a;
				return *this;
			}
			...
			Color3<T> operator* (T a) const {
				return Color3(*static_cast<const Vec3<T>*>(this) * a);
			}
			Color3<T> operator* (const Color3& c) const {
				return Color3(*static_cast<const Vec3<T>*>(this) * static_cast<const Vec3<T>&>(c));
			}
			...
			Color3<T> operator- () const {
				return Color3(-(*static_cast<const Vec3<T>*>(this)));
			}
			const Color3<T>& negate() {
				static_cast<Vec3<T>*>(this)->negate();
				return *this;
			}
			...
			Color3 pow(const Color3& exp) const {
				Color3 res;
				for (int i = 0; i < 3; ++i)
					res[i] = (*this)[i] > static_cast<T>(0) ? Math::MathT<T>::pow((*this)[i], exp[i]) : static_cast<T>(0);
				return res;
			}
	...
	template <typename T>
	inline Color3<T> exp(const Color3<T> & c) {
	...
	template <typename T>
	inline Color3<T> fastExp(const Color3<T> & c) {
	...
	template <typename T>
	inline Color3<T> log(const Color3<T> & c) {
	...
	inline Color3<T> powf(const Color3<T> & c, T e) {
	...
	template <typename T, typename S>
	inline Color3<T> operator * (S x, const Color3<T> &v) {
	...
	class Color4 : public Vec4<T> {
		public:
			...
			const Color4<T>& operator+= (const Color4<T>& c) {
				*static_cast<Vec4<T>*>(this) += c;
				return *this;
			}
			const Color4<T>& operator+= (T a) {
				*static_cast<Vec4<T>*>(this) += a;
				return *this;
			}

			Color4<T> operator- (const Color4<T>& c) const {
				return Color4(*static_cast<const Vec4<T>*>(this) - static_cast<const Vec4<T>&>(c));
			}
			...
			Color4<T> operator* (T a) const {
				return Color4(*static_cast<const Vec4<T>*>(this) * a);
			}
			Color4<T> operator* (const Color4& c) const {
				return Color4(*static_cast<const Vec4<T>*>(this) * static_cast<const Vec4<T>&>(c));
			}
			...
			Color4<T> operator- () const { return Color4(-(*static_cast<const Vec4<T>*>(this))); }
			const Color4<T>& negate() {
				static_cast<Vec4<T>*>(this)->negate();
				return *this;
			}
	...
	inline void	XYZToRGB(float *R, float *G, float *B, float X, float Y, float Z) {
	...
	inline void RGBToXYZ(float *X, float *Y, float *Z, float R, float G, float B) {
