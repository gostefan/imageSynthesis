...
namespace Math {
	...
	class RGBE {
		public:
    		...
    		RGBE(const Color3f& c) {
				Color3ToRGBE(&r, c);
			}
			RGBE(const RGBE& rgbe) : r(rgbe.r), g(rgbe.g), b(rgbe.b), e(rgbe.e) { }
	...
	inline void RGBEToFloat(float *r, float *g, float *b, const unsigned char rgbe[4]) {
	...
	inline void floatToRGBE(unsigned char rgbe[4], float red, float green, float blue) {
		float v;
		int e;
	...
	inline void Color3ToRGBE(unsigned char rgbe[4], const Color3f& c) {
		floatToRGBE(rgbe, c[0], c[1], c[2]);
	}

	inline void RGBEToColor3(Color3f* result, const unsigned char rgbe[4]) {
