...
namespace {
	const char* NAME = "Brushed Metal";
}

BrushedMetalWarping::BrushedMetalWarping(float exp1, float exp2) : 
		exp1(exp1), exp2(exp2) { }

Vec3f BrushedMetalWarping::warp(Vec2f sample) const {
...
float BrushedMetalWarping::pdf(Vec2f coord) const {
...
const char* BrushedMetalWarping::getName() const {
	return NAME;
}